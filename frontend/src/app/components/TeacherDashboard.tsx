      ...
      <ContainerScroll
        titleComponent={
          <div className="space-y-4">
            ...
            <div className="inline-flex relative z-50 -translate-y-4 items-center gap-3 px-5 py-3 rounded-2xl border border-purple-400/20 bg-[#101827] shadow-2xl backdrop-blur-md">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-purple-400/40 to-fuchsia-500/40 border border-purple-300/30 text-sm font-bold text-white">
                {initials}
              </div>
              <div className="text-left">
                <p className="text-sm font-semibold text-white">{profileName}</p>
                <p className="text-xs text-purple-300/70">{profileRole} · {profileId}</p>
              </div>
            </div>
          </div>
        }
      >
        ...
        <div className="relative w-full h-full flex overflow-hidden rounded-2xl bg-[#0d1420]">
          ...
          <div className="hidden md:flex flex-col justify-center w-[300px] shrink-0 px-7 py-8 border-r border-white/10 bg-[#101827]/80 backdrop-blur-sm">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-400/30 to-fuchsia-500/30 border border-purple-300/30 text-2xl font-bold mb-5 text-white">
              {initials}
            </div>

            <h3 className="text-lg font-bold text-white mb-1">{profileName}</h3>
            <p className="text-xs text-purple-300/70 mb-6">{profileId}</p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <div className="flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
                  <p className="text-[10px] uppercase tracking-widest text-slate-500 mb-0.5">Курсов</p>
                  <p className="text-sm text-slate-200">{classes.length}</p>
                </div>
                <div className="flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
                  <p className="text-[10px] uppercase tracking-widest text-slate-500 mb-0.5">Студентов</p>
                  <p className="text-sm text-slate-200">{totalStudents}</p>
                </div>
              </div>
              <div className="rounded-xl border border-purple-400/20 bg-purple-400/10 px-4 py-2.5">
                <p className="text-[10px] uppercase tracking-widest text-purple-400/70 mb-0.5">Статус</p>
          ...
          <div className="absolute bottom-4 left-4 right-4 md:hidden">
            <div className="rounded-xl border border-purple-300/20 bg-[#101827]/90 p-4 backdrop-blur-md">
              <div className="flex items-center gap-3 mb-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-purple-400/30 to-fuchsia-500/30 border border-purple-300/30 text-sm font-bold text-white">
                  {initials}
                </div>
                <div>
                  <p className="text-sm font-bold">{profileName}</p>
                  <p className="text-xs text-purple-300/70">{profileId}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
                <p><span className="text-slate-500">Email:</span><br />{profileEmail}</p>
                <p><span className="text-slate-500">Студентов:</span><br />{totalStudents}</p>
              </div>
            </div>
          </div>
        </div>
